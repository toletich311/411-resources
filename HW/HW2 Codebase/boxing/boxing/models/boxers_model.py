...
class Boxer:
    ...
    def __post_init__(self):
        ...
        logger.info(f"Boxer {self.name} has been initialized.")
...
def create_boxer(name: str, weight: int, height: int, reach: float, age: int) -> None:
    ...
    """
    logger.info(f"Creating boxer: {name} and adding their attributes")
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
            logger.info(f"Boxer {name} has been successfully created.")
        ...
        logger.error(f"Database error while creating {name}: {e}")
        raise e
...
def delete_boxer(boxer_id: int) -> None:
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
            logger.info(f"Boxer with ID {boxer_id} has been successfully deleted.")
...
def get_leaderboard(sort_by: str = "wins") -> List[dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Leaderboard successfully retrieved.")
        return leaderboard
...
def get_weight_class(weight: int) -> str:
    ...
    """
    logger.info(f"Trying to determine the weight class for: {weight}")
    if weight >= 203:
    ...
    logger.info(f"Weight class for {weight} is: {weight_class}")
    return weight_class
...
def update_boxer_stats(boxer_id: int, result: str) -> None:
    ...
    Raises:
        ...
        sqlite3.Error: If a database error occurs.
    """
    logger.info(f"Updating stats for boxer with ID: {boxer_id} and result: {result}")
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
            logger.info(f"Stats for boxer with ID {boxer_id} has been successfully updated.")

    except sqlite3.Error as e:
        logger.error(f"Database error while updating stats for boxer with ID {boxer_id}: {e}")
        raise e
